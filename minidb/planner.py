...
from .parser import SelectQuery, WhereClause, Condition
...
class QueryPlan:
    ...
    use_index_for: Optional[str] = None  # The specific condition using index
...
class QueryPlanner:
    ...
    def _find_index_condition(self, where: WhereClause) -> Optional[tuple]:
        """
    ...
    def _check_condition_for_index(self, condition) -> Optional[tuple]:
        ...
        if isinstance(condition, Condition):
            ...
            if self.table.has_index(column):
                # Only equality and range operators can use index
                if operator in ('=', '<', '<=', '>', '>='):
    ...
    def get_row_ids_for_condition(self, condition: Condition) -> Optional[set[int]]:
        """
